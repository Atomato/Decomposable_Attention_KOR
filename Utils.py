...
sp = SentencepieceTokenizer("./SNLI/raw data/kobert_news_wiki_ko_cased-1087f8699e.spiece")
...
def build_vocab(dataPath, vocabPath, threshold = 0):
    """
...
def sentence2Index(dataPath, vocabDict, maxLen = 100):
    """
...
def convert_embeddings(srcPath_list, srcPath_emb, dstPath):
    """
    :param srcPath_list: path of source word list
    :param srcPath_emb: path of source word embeddings 
    :param dstPath: path of output
    """
    vocab = {}
    id = 0
    wrongCnt = 0
    with open(srcPath_list, 'r', encoding = 'utf-8') as fin_list, \
            open(srcPath_emb, 'r', encoding = 'utf-8') as fin_emb:
        lines_l = fin_list.readlines()
        lines_e = fin_emb.readlines()
        wordNums = len(lines_e)
        line_e = lines_e[0].strip().split()
        vectorDims = len(line_e)
        embeddings = np.zeros((wordNums, vectorDims), dtype = np.float32)
        for line_l, line_e in zip(lines_l, lines_e):
            line_l = line_l.strip() # remove \n
            items = line_e.strip().split()
            if len(items) != vectorDims:
                wrongCnt += 1
                print(line_l, line_e)
                continue
            if line_l in vocab:
                ...
                continue
            vocab[line_l] = id
            embeddings[id] = [float(v) for v in items]
            id += 1
        ...
        print('valid embedding nums : {0}, embeddings shape : {1},'
              ' wrong format embedding nums : {2}, total embedding nums : {3}'.format(len(vocab),
                                                                                      ...
                                                                                      wrongCnt,
                                                                                      wordNums))
        print('Original embeddings has been converted from {0}, {1} to {2}'.\
            format(srcPath_list, srcPath_emb, dstPath))
...
if __name__ == '__main__':
    ...
    convert_embeddings('./SNLI/raw data/word_list.txt', './SNLI/raw data/word_emb.txt', 
        './SNLI/clean data/embeddings.pkl')
